...
def _build_ollama_prompt(text: str, task: Optional[str] = None) -> str:
    ...
    return (
        ...
        "Änderungsbescheid, Steuerbescheid, Bescheid, Mahnung, Kündigung, Rechnung, Formular, "
        "Information - or null if none clearly applies. Mahnbescheid is a formal, court-issued "
        "payment order (gerichtliches Mahnverfahren); Mahnung is only an informal payment reminder "
        "with no court involved - these are two different values, never use one when the text "
        "supports the other. "
        ...
        "or deadlines). This category also covers a contract ending under any other German wording "
        "for the same thing, even when the literal word \"Kündigung\" never appears - "
        "\"Vertragsaufhebung\", \"Vertragsende\", \"Vertragsauflösung\", and \"Aufhebungsvertrag\" "
        "are all Kündigung too (e.g. \"Ihr Vertrag ... endet am 10.12.2025\" is a Kündigung, not "
        "Information, even though it never uses the word \"kündigen\" itself). Anhörung "
        ...
        "that applies instead), never Information. "
        "Formular is a blank or partially-filled form that the READER is meant to fill in and send "
        "back (e.g. \"Antragsformular\", \"Fragebogen\", a Beitrittserklärung with signature fields), "
        "not a letter that was already decided or billed by the sender - use Formular even if the "
        "form's own subject matter is insurance, a loan, or a credit card (e.g. an "
        "\"Antragsformular für vorübergehende Arbeitsunfähigkeit\" under a Restschuldversicherung "
        "is a Formular the reader submits, never a Rechnung, even though it mentions a Kreditkarte, "
        "Bank, or Versicherung - those words describe what the form is ABOUT, not a bill being sent). "
        "Never use Rechnung unless the text itself states a concrete amount the reader owes and a "
        "concrete payment deadline for it - a document that merely mentions a bank, credit card, or "
        "insurance product by name is not automatically a Rechnung. "
        "Reserve Information for letters that only "
        ...
        "der 27.04.2026\", \"endet am 10.12.2025\"), or a change's effective date - formatted "
        "YYYY-MM-DD, only if an exact "
        ...
        f"pay, submit documents, appeal); {PAYMENT_REQUESTED_KEY} is true only if the text states a "
        "concrete amount owed AND asks the reader to pay it by a stated or implied deadline - merely "
        "mentioning a bank, credit card, insurance policy, premium, or account number is NOT a "
        f"payment demand by itself; {OBJECTION_RIGHT_KEY} is true only if the text literally uses one of the "
